...
my_patient = Patient(name=[{"given": ["John"], "family": "Doe"}])
family_names = expression.values(my_patient)

assert family_names == ["Doe"]
```
...
gender = my_patient.fhirpath_single("Patient.gender")
...
first_name = my_patient.fhirpath_first("Patient.name.given")
...
# Engine interface
expression = fhirpath.parse("Patient.name.family")
engine_result = expression.values(my_patient)

# Resource interface (more convenient)
resource_result = my_patient.fhirpath_values("Patient.name.family")

assert engine_result == resource_result
```
...
# Result: 2 if my_patient has 2 given names
```
...
# Check if my_patient has a phone number
