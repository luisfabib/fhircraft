...
class FHIRStructureMapper:
    ...
    def map(
        ...
        try:
            ...
            raise Exception(f"Mapping execution failed: {e}") from e
    ...
    def parse(self, script: str) -> StructureMapUnion:
        ...
        try:
            ...
            raise Exception(f"Failed to parse mapping script: {e}") from e

    def validate(self, script: str) -> bool:
        """
        Check whether a FHIR mapping language script is syntactically valid.

        Args:
            script: FHIR mapping language script to validate.

        Returns:
            ``True`` if the script parses without errors, ``False`` otherwise.
        """
        try:
            self._parser.parse(script)
            return True
        except Exception:
            return False
