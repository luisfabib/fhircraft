...
def load_structure_map(source: Union[str, Path, Dict[str, Any]]) -> StructureMap:
    """
    Convenience function to load a StructureMap from various sources.

    Args:
        source: File path, URL, or dictionary containing the StructureMap

    Returns:
        StructureMap object
    """
    mapper = FHIRMapper()
    return mapper.load_structure_map(source)


def parse_mapping_script(script: str) -> StructureMap:
    """
    Convenience function to parse a FHIR mapping language script.

    Args:
        script: FHIR mapping language script

    Returns:
        StructureMap object
    """
    mapper = FHIRMapper()
    return mapper.parse_mapping_script(script)


def execute_mapping(
    mapping: Union[str, StructureMap, Dict[str, Any]],
    source_data: Union[BaseModel, Dict[str, Any], Tuple[Union[BaseModel, dict], ...]],
    target_data: Optional[
        Union[BaseModel, Dict[str, Any], Tuple[Union[BaseModel, dict], ...]]
    ] = None,
    group: Optional[str] = None,
    ...
    """
    Convenience function to execute a mapping transformation.

    Args:
        mapping: StructureMap, script, or dict containing the mapping
        source_data: Source data to transform
        target_data: Optional pre-existing target objects
        group: Optional specific group to execute

    Returns:
        MappingResult with transformed targets
    """
    mapper = FHIRMapper()
    return mapper.execute_mapping(mapping, source_data, target_data, group=group)
