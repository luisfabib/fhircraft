...
with open(os.path.join(PATH, "profiles-types.json"), "r", encoding="utf-8") as file:
    ...
    structure_definitions += [
        resource["resource"]
        for resource in datatypes_bundle["entry"]
        if not hasattr(primitives, capitalize(resource["resource"]["name"]))
    ]
with open(os.path.join(PATH, "profiles-resources.json"), "r", encoding="utf-8") as file:
    ...
    structure_definitions += [
        resource["resource"]
        for resource in datatypes_bundle["entry"]
        if resource["resource"]["name"] in ["Resource", "DomainResource"]
    ]
...
for structure_definition in structure_definitions:
    resource = factory.construct_dataelement_model(
...
with open(
    os.path.join(DESTINATION, "complex_types.py"), "+w", encoding="utf-8"
) as file:
