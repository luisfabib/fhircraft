...
from fhircraft.fhir.mapping.StructureMap import StructureMap, StructureMapParameter, StructureMapStructure, StructureMapTarget, StructureMapConst, StructureMapGroup, StructureMapInput, StructureMapRule, StructureMapDependent, StructureMapSource
...
def _parse_StructureMapParameter(value: any) -> StructureMapParameter:
    return StructureMapParameter(
        ...
        valueDate=value if isinstance(value, literals.Date) else None,
        valueDateTime=value if isinstance(value, literals.DateTime) else None,
        valueTime=value if isinstance(value, literals.Time) else None,
    )
...
class FhirMappingLanguageParser:
    ...
    def __init__(self, debug=False, lexer_class=None):
        ...
        self.debug = debug
        self.lexer_class = lexer_class or FhirMappingLanguageLexer # Crufty but works around statefulness in PLY
    ...
    def p_error(self, t):
        if t is None:
            raise FhirMappingLanguageParserError(f'FHIR Mapping Language parser error near the end of string "{self.string}"!')
        raise FhirMappingLanguageParserError(f'FHIR Mapping Language parser error at {t.lineno}:{t.col} - Invalid token "{t.value}" ({t.type}):\n{_underline_error_in_fhir_path(self.string, t.value, t.col)}')
    ...
    def p_parameter(self, p):
        ...
        p[0] = StructureMapInput(mode=p[1], name=p[2], type=p[4] if len(p)==5 else None)
    ...
    def p_rule(self, p):
        ...
        p[0] = StructureMapRule(
            ...
            rule=p[4].get('contained_rules') if p[2]=='->' else p[2].get('contained_rules'),
    ...
    def p_dependent(self, p):
        ...
        p[0] = [
            StructureMapDependent(name=invocation['name'], parameter=[
                _parse_StructureMapParameter(value) for value in invocation['parameter']
            ])
