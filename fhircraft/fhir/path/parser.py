...
class FHIRPathParser:
    ...
    lexer_class: FHIRPathLexer | type = FHIRPathLexer
    """The lexer class used by the parser. Defaults to FHIRPathLexer."""

    lexer: FHIRPathLexer | None = None
    """The lexer instance used by the parser."""

    cache: dict[str, FHIRPath] = {}
    """A cache of parsed FHIRPath expressions to avoid re-parsing the same expression."""
    ...
    def parse(self, input_string: str) -> FHIRPath | Any:
        ...
        if self.input_string not in self.cache:
            self.cache[self.input_string] = self.parse_token_stream(
                self.lexer.tokenize(self.input_string)
            )
        return self.cache[self.input_string]

    @property
    def cache_info(self) -> dict:
        """Return basic cache statistics (size and stored expression strings)."""
        return {"size": len(self.cache), "expressions": list(self.cache)}
        ...
        def check(args, function, nargs):
            if args[1] == function:
                ...
                if nprovided not in ensure_list(nargs):
                    raise FHIRPathParsingError(
                        f"FHIRPath parser error at {p.lineno(1)}:{p.lexpos(1)}: "
                        f"Function {function}() requires {nargs} arguments, "
                        f"but {nprovided} were provided."
                        f"\n{_underline_error_in_fhir_path(self.input_string, function, p.lexpos(1))}"
                    )
                return True
            return False
