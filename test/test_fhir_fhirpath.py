...
fhirpath_find_test_cases = (
    ...
    ('Observation.fhir_comments', observation, ['A',"B","C"]),
...
fhirpath_update_test_cases = (
    ...
    ('Observation.fhir_comments', ['A','B','C'], lambda obs: obs.fhir_comments),
...
def test_fhirpath_update_or_create(path_string, update_value, getattr_fcn):
    Observation.Config.validate_assignment = False
    new_observation = get_fhir_model_class(path_string.split('.',1)[0]).construct()
    parse(path_string).update_or_create(new_observation, update_value)
    assert getattr_fcn(new_observation) == update_value