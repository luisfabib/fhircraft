...
# ============================================================================
# CONDITION AND ASSERTION TESTS
# ============================================================================


class TestRuleSourceConditions:
    """Tests for where and assertion condition checking."""

    @pytest.fixture(autouse=True)
    def setup_conditions(
        self, rule_source_factory, mock_source_definition, mock_rule, mock_scope
    ):
        """Setup rule source with conditions for testing."""
        mock_source_definition.condition = "active = true"
        mock_source_definition.check = "name.exists()"

        self.mock_condition_expr = Mock()
        self.mock_assertion_expr = Mock()

        with patch("fhircraft.fhir.path.fhirpath.parse") as mock_parse:
            mock_parse.side_effect = [
                self.mock_condition_expr,
                self.mock_assertion_expr,
            ]
            self.rule_source = rule_source_factory(
                mock_source_definition, mock_rule, patch_literals=False
            )

        self.mock_scope = mock_scope

    @pytest.mark.parametrize(
        "condition_result,expected", [(True, True), (False, False)]
    )
    def test_check_where_condition(self, condition_result, expected):
        """Test where condition check with different results."""
        mock_replaced_expr = Mock()
        mock_replaced_expr.single.return_value = condition_result

        with patch.object(
            self.rule_source,
            "_replace_mapping_scope_elements",
            return_value=mock_replaced_expr,
        ):
            result = self.rule_source._check_where_condition(self.mock_scope)

            assert result is expected
            mock_replaced_expr.single.assert_called_once_with(
                self.mock_scope.get_instances()
            )

    @pytest.mark.parametrize(
        "assertion_result,expected", [(True, True), (False, False)]
    )
    def test_check_assertion_condition(self, assertion_result, expected):
        """Test assertion condition check with different results."""
        mock_replaced_expr = Mock()
        mock_replaced_expr.single.return_value = assertion_result
        ...
        with patch.object(
            ...
            result = self.rule_source._check_assertion_condition(self.mock_scope)
